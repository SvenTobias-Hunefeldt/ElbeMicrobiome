...
Mantel_IndPP_Comm.df$PhysicochemicalParameter = gsub("Temperature_TBDHereon", "Temperature (°C)", Mantel_IndPP_Comm.df$PhysicochemicalParameter)
Mantel_IndPP_Comm.df$PhysicochemicalParameter = gsub("dCO2_uM", "dCO2 (µM)", Mantel_IndPP_Comm.df$PhysicochemicalParameter)
Mantel_IndPP_Comm.df$PhysicochemicalParameter = gsub("dCH4_nM", "dCH4 (nM)", Mantel_IndPP_Comm.df$PhysicochemicalParameter)

Mantel_IndPP_Comm.df$PhysicochemicalParameter = gsub("O2_uM", "dO2 (µM)", Mantel_IndPP_Comm.df$PhysicochemicalParameter)
...
Mantel_IndPP_Comm.df$PhysicochemicalParameter = gsub("TEP_um2perL", "TEP (µm2 L-1)", Mantel_IndPP_Comm.df$PhysicochemicalParameter)
Mantel_IndPP_Comm.df$PhysicochemicalParameter = gsub("CSP_um2perL", "CSP (µm2 L-1)", Mantel_IndPP_Comm.df$PhysicochemicalParameter)
Mantel_IndPP_Comm.df$PhysicochemicalParameter = gsub("Density_mgperL_um2perL", "Density (mgL-1 µm2L-1)", Mantel_IndPP_Comm.df$PhysicochemicalParameter)
...
ectD is required for the main ectoine hydroxylase activity in C. salexigens. – stress response to temperature and salt (García-Estepa et al., 2006)
...
pmoA is a large component of the methane metabolism pathway and represent a large methane sink in the ecosystem - regulated by CH4 abundance (Rodríguez et al., 2019)
Pepck – glucose production (gluconeogenesis) and produces CO2 - decreased expression during DNA damage events (Kim et al., 2011)
Fdh – reversible formate <-> CO2, de novo nucleotide (purine) biosynthesis, produces methane from formate and other substrates and is therefore located in methanogens, but expression is controlled by H2 concentrations rather than formate (Wood et al., 2003)
...
GGCX (the only known vitamin K post-translational modification, produces γ-carboxyglutamate, a vital precursor to many proteins including those involved in cellular growth, survival, and signalling (Shearer & Newman, 2014; Shearer & Okano, 2018)). 
...
O2_uM.plt = ggplot(PP_totest.df, aes(x = station_km, y = O2_uM, colour = Sample_date, group = Sample_date)) + 
  ...
  My_Theme+
  xlab("Elbe km")+
  ylab(expression("Dissolved O"[2]~"(µM)"))
O2_uM.plt
...
Results_PPvsPP_sign.df$Var1 = gsub("Temperature_TBDHereon", "Temperature (°C)", Results_PPvsPP_sign.df$Var1)
Results_PPvsPP_sign.df$Var1 = gsub("dCO2_uM", "dCO2 (µM)", Results_PPvsPP_sign.df$Var1)
Results_PPvsPP_sign.df$Var1 = gsub("dCH4_nM", "dCH4 (nM)", Results_PPvsPP_sign.df$Var1)

Results_PPvsPP_sign.df$Var1 = gsub("O2_uM", "dO2 (µM)", Results_PPvsPP_sign.df$Var1)
...
Results_PPvsPP_sign.df$Var1 = gsub("TEP_um2perL", "TEP (µm2 L-1)", Results_PPvsPP_sign.df$Var1)
Results_PPvsPP_sign.df$Var1 = gsub("CSP_um2perL", "CSP (µm2 L-1)", Results_PPvsPP_sign.df$Var1)
Results_PPvsPP_sign.df$Var1 = gsub("Density_mgperL_um2perL", "Density (mgL-1 µm2L-1)", Results_PPvsPP_sign.df$Var1)
...
Results_PPvsPP_sign.df$Var2 = gsub("Temperature_TBDHereon", "Temperature (°C)", Results_PPvsPP_sign.df$Var2)
Results_PPvsPP_sign.df$Var2 = gsub("dCO2_uM", "dCO2 (µM)", Results_PPvsPP_sign.df$Var2)
Results_PPvsPP_sign.df$Var2 = gsub("dCH4_nM", "dCH4 (nM)", Results_PPvsPP_sign.df$Var2)

Results_PPvsPP_sign.df$Var2 = gsub("O2_uM", "dO2 (µM)", Results_PPvsPP_sign.df$Var2)
...
Results_PPvsPP_sign.df$Var2 = gsub("TEP_um2perL", "TEP (µm2 L-1)", Results_PPvsPP_sign.df$Var2)
Results_PPvsPP_sign.df$Var2 = gsub("CSP_um2perL", "CSP (µm2 L-1)", Results_PPvsPP_sign.df$Var2)
Results_PPvsPP_sign.df$Var2 = gsub("Density_mgperL_um2perL", "Density (mgL-1 µm2L-1)", Results_PPvsPP_sign.df$Var2)
...
CO2.plt = ggplot(PP_totest.df, aes(x = station_km, y = dCO2_uM, colour = Sample_date, group = Sample_date)) + 
  ...
  My_Theme+
  xlab("Elbe km")+
  ylab(expression("Dissolved CO"[2]~"(µM)"))
...
#Calvin cycle
#the reverse tricarboxylic acid (rTCA) cycle, the 3-hydroxypropionate (3HP) bi-cycle, the 4-hydroxybutyrate/3-hydroxypropionate (4HB/3HP) cycle, the dicarboxylate/4-hydroxybutyrate (DC/4HB) cycle, the reductive acetyl-CoA pathway (Wood–Ljungdahl pathway—WLP), and the reductive glycine pathway
...
#Malonyl‐CoA is reduced to 3‐hydroxypropionate in a NADPH‐dependent reaction, which is catalyzed by the bifunctional enzyme malonyl‐CoA reductase. This key enzyme comprises malonate semialdehyde and 3‐hydroxypropionate dehydrogenase activity (Hügler et al., 2002).
#Hydroxypropionate is further converted to propionyl‐CoA by a trifunctional enzyme, propionyl‐CoA synthase, the second key enzyme of the cycle.
...
#reductive acetyl-CoA pathway (Wood–Ljungdahl pathway—WLP) - K00193 (cdhC) + K00194 (cdhD) + K00197 (cdhE) + K00195 (cdhB) + K00196 (cooF) + K00198 (cooS) + K00192 (cdhA)
